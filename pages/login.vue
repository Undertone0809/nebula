<template>
  <el-row class="wrapper verify" style="background-color: #F6F7F8;padding-top: 10%;">
    ...
    <el-col :xs="24" :sm="12" :xl="12" class="intro vditor-reset verify__sign">
      <div>
        <h2>欢迎来到 Forest</h2>
        <p><a rel="nofollow" href="/">Forest</a> 是一个知识学习交流平台，专注于计算机知识学习。</p>
        <p>我们正在构建一个社区。大家在这里相互信任，以平等 • 自由 • 奔放的价值观进行分享交流。最终，希望大家能够找到与自己志同道合的伙伴，共同成长。</p>
        <p>最后请大家共同爱护这个自由的交流环境，相信这里一定是你注册过的所有社区中用户体验最好的 😍</p>
      </div>
    </el-col>
    <el-dialog
      title="找回密码"
    ...
    login() {
      ...
      _ts.$refs.user.validate((valid) => {
        if (valid) {
          ...
          _ts.$axios.$post('/api/console/login', data).then(res => {
            _ts.$set(_ts, 'loginLoading', false);
            if (res) {
              if (res.message) {
